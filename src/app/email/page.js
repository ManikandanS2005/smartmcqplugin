    ...
    try {
      const res = await fetch("/api/email", {
        ...
        body: JSON.stringify({
          ...
          accessToken: session?.accessToken, // ✅ pass the token
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center p-6">
      <div className="bg-gray-800 shadow-lg rounded-xl p-6 w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-center">Send Google Form via Email</h1>
        ...
        <button
          onClick={handleSend}
          disabled={loading}
          className={`w-full py-2 rounded-lg ${loading ? "bg-gray-600" : "bg-blue-600 hover:bg-blue-700"}`}
        >
